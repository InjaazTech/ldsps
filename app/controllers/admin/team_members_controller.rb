class Admin::TeamMembersController < Admin::BaseController
  ...
  def index
    @team_members = TeamMember.order(order_position: :asc).page(@page_index)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @team_member.save
        format.html { redirect_to admin_team_member_path(@team_member), notice: t('admin.success') }
  ...
  def destroy
    @team_member.destroy
    respond_to do |format|
      format.html { redirect_to admin_team_members_url, notice: t('admin.success') }
